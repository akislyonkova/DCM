...
NRDM<-function(Qmatrix,scale.num,save.path=getwd(),save.name="NRDM"){
  nc<-ncol(Qmatrix)
  nr<-nrow(Qmatrix)
  ...
  for(loopi in 1:nr){
    for( loopc in 1:nclass){
      ...
      for( loops in 3:3){
        Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=',
                                         paste(Kernel.exp.list[[2]][loopi,loopc]),"+", paste(Kernel.exp.list[[3]][loopi,loopc]),';\n',sep='')
      }
      for( loops in 4:4){
        Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=',
                                         paste(Kernel.exp.list[[2]][loopi,loopc]),"+", paste(Kernel.exp.list[[3]][loopi,loopc]), "+", paste(Kernel.exp.list[[4]][loopi,loopc]),';\n',sep='')
      }
      for( loops in 5:5){
        Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=',
                                         paste(Kernel.exp.list[[2]][loopi,loopc]),"+", paste(Kernel.exp.list[[3]][loopi,loopc]), "+", paste(Kernel.exp.list[[4]][loopi,loopc]),"+", paste(Kernel.exp.list[[5]][loopi,loopc]),';\n',sep='')
      }
    }
  }
