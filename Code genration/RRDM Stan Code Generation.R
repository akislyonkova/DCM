...
RRDM<-function(Qmatrix,scale.num,save.path=getwd(),save.name="RRDM"){
  ...
  #print(Kernel.exp.list) - на этом этапе все как надо 
...
#Kernel.exp.list.copy<-Kernel.exp.list
    ...
    #for (j in 1:nr){
      #for( z in 1:nclass){
        #Kernel.exp.list[[i]][j,z]<-str_replace_all(Kernel.exp.list[[i]][j,z],Kernel.exp.list.copy[[i]][j,1],
                                                   #paste(Kernel.exp.list.copy[[i]][j,1],i-1,sep=''))
        
      #}
    #}
  #}
  
  #print(Kernel.exp.list)
  ...
  for(loopi in 1:nr){
    for( loopc in 1:nclass){
      ##TODO: for (loops in 1:scale.num)
      loops = 1
      Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=0;\n',sep='')
      
      loops = 2
      Reparm[loopi,loopc,loops]<-paste('PImat[', loopi,',',loopc,'][',loops,']=',
                                       paste(loops-1), 
                                       paste('*('),
                                       paste(Kernel.exp.list[[2]][loopi,loopc]), 
                                       paste(") - step_1"), ';\n',sep='')
      loops = 3
      Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=',
                                       paste(loops-1), 
                                       paste('*('),
                                       paste(Kernel.exp.list[[3]][loopi,loopc]), 
                                       paste(") - step_1 - step_2"), ';\n',sep='')
      loops = 4
      Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=',
                                       paste(loops-1), 
                                       paste('*('),
                                       paste(Kernel.exp.list[[4]][loopi,loopc]), 
                                       paste(") - step_1 - step_2 - step_3"), ';\n',sep='')
      loops = 5
      Reparm[loopi,loopc,loops]<-paste('  PImat[',loopi,',',loopc,'][',loops,']=',
                                       paste(loops-1), 
                                       paste('*('),
                                       paste(Kernel.exp.list[[5]][loopi,loopc]), 
                                       paste(") - step_1 - step_2 - step_3 - step_4"), ';\n',sep='')
  }
}
# добавляет числа после ноликов для unconstraint list
...
parm.spec<-paste(c('parameters{
                     simplex[Nc] Vc;\n ',
                   paste0(Constrain.List),
                   paste0(Unconstrain.List),
                   paste0(StepList),'}\n')
                 ,collapse='')
